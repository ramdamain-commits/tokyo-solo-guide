...
調査は `data/sources.json` の承認済みソースを基点にする。priority 1 を第1ソース、type:reference は裏取り専用。
...
> 基点ソース: data/sources.json の priority 1〜2（Tokyo Art Beat / Go Tokyo / 各館公式）。
> スキーマ:
...
**差し込み前の照合（必須）**: researcher の出力は、全件の出典が公式か／会期の年が今年か／料金が現行かを公式ページで確認する。3年周期の催し（例: 横浜トリエンナーレ）や毎年同じ日に開く行事は前年情報が混じりやすく、観光施設の料金は値上げ前の値が返りやすい（2026-09 に月次3件・常設4件の誤りを検出）。確認できない項目は入れない。
...
## 4. 検索ソースの追加・削除
`data/sources.json` は「**調査の基点レジストリ**」であり、各スポットの `source`（evergreen.json / monthly.json の item に直書きされた出典URL）とは独立している。sources.json からソースを削除しても item の `source` は残るので、削除時は item 側の出典も陳腐化していないか別途確認する。
